...
This section gives the full per-cluster descriptions summarized in Chapter~\ref{chap:experiments}. Sections~\ref{subsec:awa2_ddc}--\ref{subsec:apy_ddc} give the \texttt{ddc} mode (ILP applied to the K-means clustering) and Sections~\ref{subsec:awa2_ddeccs}--\ref{subsec:apy_ddeccs} the \texttt{ddeccs} mode (ILP applied to the consensus clustering), both at the representative seed.

\subsection{AwA2 Cluster Descriptions (ddc mode, K = 50)}
\label{subsec:awa2_ddc}
...
\midrule
K-means (ensemble member) & scikit-learn & Elkan algorithm, $n\_init = \text{auto}$, $max\_iter = 300$ \\
