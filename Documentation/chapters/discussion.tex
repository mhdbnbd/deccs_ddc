...
DDC achieves NMI $\approx$ 0.78 on AwA, while our approach achieved only NMI=0.012 on the full AwA2 dataset. Understanding this gap is crucial:
...
DDC uses a carefully designed architecture that has been validated on AwA. Our adaptation required modifications to integrate with DECCS's framework, potentially introducing suboptimal design choices.
...
\begin{itemize}
    ...
    \item Task interference during multi-objective optimization, where gradients from reconstruction, tag prediction, and consensus losses conflict, preventing the model from learning cluster-aligned representations
    \item Loss imbalance, where the reconstruction objective dominates and the model does not sufficiently optimize for clustering structure
\end{itemize}
