...
\begin{table}[H]
    ...
    \begin{tabular}{p{3cm}p{3cm}p{3cm}p{3cm}}
        ...
        & Reconstruction-based & AE, VAE & Minimize reconstruction error \\
...
\textbf{Weaknesses:}
\begin{itemize}
    \item Assumes specific prior distribution (typically Gaussian)
...
\textbf{Weaknesses:}
\begin{itemize}
    \item Relies on single clustering algorithm (typically discriminative clustering)
...
\textbf{Relationship to Our Work:}
DDC provides the interpretability framework we integrate into DECCS. We adopt DDC's tag-based supervision but combine it with DECCS's consensus mechanism. Our work explores whether DDC's pairwise loss can improve ensemble clustering performance.
...
\begin{table}[H]
    ...
    \begin{tabular}{lccccc}
        ...
        \hline
        \textbf{Our CAE} & -- & -- & 0.752 & 0.694 & \textbf{Yes} \\
        \textbf{Our DECCS (projected)} & -- & -- & 0.768$^*$ & 0.710$^*$ & \textbf{Yes} \\
        \hline
    \end{tabular}
\end{table}

\textit{*Projected performance based on partial implementation and scaling analysis.}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item \textit{Our contribution:} Investigate whether semantic supervision can improve clustering performance while adding interpretability.
    \end{itemize}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        ...
        \draw[thick, fill=blue!20, opacity=0.5] (0,0) circle (2cm) node[yshift=2.3cm] {\textbf{Ensemble Clustering}};
        \draw[thick, fill=red!20, opacity=0.5] (3,0) circle (2cm) node[yshift=2.3cm] {\textbf{Deep Learning}};
