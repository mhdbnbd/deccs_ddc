...
The second concerns the explanation for the remaining gap. Chapter~\ref{chap:discussion} attributes it to the original DDC's task-specific feature training. That account is now too simple. On AwA2 a substantial part of the margin comes from class information entering through the pairwise term rather than from better features, and the paper-faithful configuration in fact loses to a matched k-means baseline. On aPY the faithful method does not reach the published figure at any feasible coverage value, and the reproduction sits below the frozen-feature baseline throughout. Feature adaptation may still contribute, but it is not the whole of the difference, and on aPY it is not demonstrated at all under matched conditions.
