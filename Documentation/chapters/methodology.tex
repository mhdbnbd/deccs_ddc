...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        \node (tagpred) [block, right of=embedding, xshift=3cm] {  Predictor};
        ...
        \node[draw=none, text width=2.5cm, align=left] at (4, -3) {$\mathcal{L}_{tag}$};
...
\textbf{List of semantic attributes:}
\begin{multicols}{3}
    \begin{itemize}
        \item black
        \item white
...
\begin{enumerate}
    ...
    \item \textbf{Error handling}: Skip corrupted images gracefully
\end{enumerate}
...
Our ensemble includes five diverse algorithms, following the principle of using heterogeneous clustering methods to improve robustness \citep{Yang2017}:
...
\begin{itemize}
    ...
    \item \textbf{Learning rate}: $\alpha = 10^{-3}$ (Pytorch default)
    \item \textbf{Momentum parameters}: $\beta_1 = 0.9, \beta_2 = 0.999$ (Pytorch default)
    \item \textbf{Epsilon}: $\epsilon = 10^{-8}$ (Pytorch default)
    \item \textbf{Weight decay}: None (Pytorch default)
\end{itemize}
...
\paragraph{Normalized Mutual Information (NMI)} \citep{Vinh2010}

\begin{equation}
    NMI(\pi, l) = \frac{2 \cdot I(\pi; l)}{H(\pi) + H(l)}
\end{equation}

where $I(\cdot; \cdot)$ is mutual information, $H(\cdot)$ is entropy, $\pi$ are predicted clusters, and $l$ are true labels. NMI ranges from 0 (no mutual information) to 1 (perfect correlation).


\paragraph{Adjusted Rand Index (ARI)} \citep{Hubert1985}

\begin{equation}
    ARI(\pi, l) = \frac{\sum_{ij} \binom{n_{ij}}{2} - \left[\sum_i \binom{a_i}{2} \sum_j \binom{b_j}{2}\right] / \binom{n}{2}}{\frac{1}{2}\left[\sum_i \binom{a_i}{2} + \sum_j \binom{b_j}{2}\right] - \left[\sum_i \binom{a_i}{2} \sum_j \binom{b_j}{2}\right] / \binom{n}{2}}
\end{equation}
...
\paragraph{Clustering Accuracy (ACC)} \citep{Kuhn1955}

\begin{equation}
    ACC(\pi, l) = \max_{\sigma} \frac{1}{N} \sum_{i=1}^{N} \mathds{1}[l_i = \sigma(\pi_i)]
\end{equation}

where $\sigma$ is the optimal permutation found via the Hungarian algorithm, which finds the best one-to-one mapping between predicted clusters and true labels.


\paragraph{Silhouette Score} \citep{Rousseeuw1987}
...
\begin{itemize}
\item \textbf{Deep Learning}: PyTorch 2.0+ \citep{Paszke2019}
...
\begin{itemize}
\item \textbf{CPU}: Intel Core i7 (local sample experiments)
\item \textbf{GPU}: NVIDIA GPU with CUDA support, Luke server (full scale experiments)
\end{itemize}
...
\begin{table}[H]
    \centering
    \caption{Complete configuration summary}
    \begin{tabular}{ll}
        \hline
